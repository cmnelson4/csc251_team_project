...
public class Car {
	...
	public String toString() {
		...
		String printable = String.format("%-4s  %-10d  %-6d  %-10s  $%-12.2f  $%-14.2f",
				this.getId(), this.getMileage(), this.getMpg(), (this.isSold() ? "Yes" : " No"), this.getCost(), this.getSalesPrice());

		printable = printable + (this.isSold() ?
				String.format("$%-11.2f  $%-5.2f", this.getPriceSold(), this.getProfit()) : "");

		return printable;
	}

	public String toCSVString() {
		return String.format("%s, %6d, %3d, %4s, %5.2f, %5.2f, %5.2f, %5.2f",
